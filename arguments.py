...
def Arguments():
    ...
    parser.add_argument('--N_run', dest='N_run', type=int, default=10, help='number of executions of the algorithm')
    parser.add_argument('--lr', dest='lr', type=float, default=1e-4, help='learning rate')
    parser.add_argument('--patience', dest='patience', type=int, default=20, help='number of epochs without improvement to apply early stop')
    parser.add_argument('--Mask_P_M', dest='Mask_P_M', type=eval, choices=[True, False], default=True, help='mask')
    parser.add_argument('--weights', dest='weights', type=list, default=[], help='weights for weighted cross entropy loss classes: [0, 1]')
...
def Arguments_metrics():
    parser = argparse.ArgumentParser(description='')
    parser.add_argument('--Arq', dest='Arq', type=int, default=2, help='Architecture: 1 - Segnet, 2 - Unet, 3 - DeepLab, 4 - DenseNet')
    parser.add_argument('--Npoints', dest='Npoints', type=int, default=2000, help='number of points in the curves pr vs rec')
    parser.add_argument('--dataset', type=str, choices=['Landsat8', 'Sentinel2'], default='Landsat8', help='dataset name')
    parser.add_argument('--patch_size', dest='patch_size', type=int, default=128, help= 'dimension of the extracted patches')
    parser.add_argument('--overlap_percent', dest='overlap_percent', type=float, default=0.5, help= 'overlap between classes')
